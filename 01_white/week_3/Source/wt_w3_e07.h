...
{
/*!
 * \brief The Person3 see Readme07
 */
class Person3 {
private:
...
public:
    Person3()=delete;
    explicit Person3( string fname, string lname, int year):
        born(year)
    {
        first[year]=fname;
        last[year] = lname;
...
private:
    ...
    vector<string> CleanRepetitives(vector<string> v) const
    {
        vector<string> res;
        string prev{};
        for(auto const &i: v )
    ...
    string GetHistory(int year, map<int, string> const & m) const
    {
        if(year < born){
            return {};
        }
    ...
    string BuildName(string const & f, string const & l) const
    {
        if (f.empty() && l.empty())        {
            return "Incognito";
        }
    ...
    string BuildHistory(vector<string> const &v) const
    {
        if (v.empty())
        {
            return {};
    ...
    string FullName(string const &fname, string const & lname) const
    {
        stringstream ss;
    ...
    string LatestRecord(int year, map<int, string> const & m) const
    {
    ...
    void ClearRepetitiveRecords(int year, map<int, string> & m,  string name) const
    {
        auto it=m.find(year);
        ++it;
        if (it->second == name)
